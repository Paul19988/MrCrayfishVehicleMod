                ...
                if(!stack.isEmpty())
                {
                    ...
                    for(int j = 0; j < count; j++)
                    {
                        matrixStack.pushPose();
                        {
                            ...
                            MINECRAFT.getItemRenderer().render(stack, ItemTransforms.TransformType.NONE, false, matrixStack, renderTypeBuffer, light, OverlayTexture.NO_OVERLAY, RenderObjectHelper.getModel(stack));
                        }
                        matrixStack.popPose();
                        index++;
