    ...
    public void setupTransformsAndRender(@Nullable T vehicle, PoseStack matrixStack, MultiBufferSource renderTypeBuffer, float partialTicks, int light)
    {
        matrixStack.pushPose();
        {
            ...
            if(properties.canTowTrailers())
            {
                ...
                RenderObjectHelper.renderColoredModel(this.getTowBarModel().getBaseModel(), ItemTransforms.TransformType.NONE, false, matrixStack, renderTypeBuffer, -1, OverlayTexture.NO_OVERLAY, light);
                matrixStack.popPose();
            }
